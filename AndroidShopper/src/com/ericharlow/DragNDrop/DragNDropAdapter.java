...
public final class DragNDropAdapter extends BaseAdapter implements
	...
	public DragNDropAdapter(Context context, List<ShoppingListItem> content) {
		init(context, content);
	}

	private void init(Context context, List<ShoppingListItem> content) {
		mInflater = LayoutInflater.from(context);
		mIds = new long[content.size()];
		mContent = new ArrayList<String>();
		int i = 0;
		for (ShoppingListItem shoppingListItem : content) {
			mIds[i] = shoppingListItem.getId();
			mContent.add(shoppingListItem.getProduct().getName());
		}
	}
